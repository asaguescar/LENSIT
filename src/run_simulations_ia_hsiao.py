...
from simulations.simulating_sne import glsneia_hsiao_sample

out_ia_hsiao = glsneia_hsiao_sample(outlensed.to_dict('list'), cosmo)
...
out_ia_hsiao['t0'] = np.random.uniform(starting_date, ending_date, size=len(out_ia_hsiao['z']))

ia_hsiao = pd.DataFrame(out_ia_hsiao)
sample = GLSNeIa_hsiao.from_data(ia_hsiao[ia_hsiao.Lensed==1])
...
outputfilename = path+'dset_ia_hsiao_'+str(size)+'_'+str(i)+'.pkl'
