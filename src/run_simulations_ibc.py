...
from simulations.simulating_sne import glsneibc_sample

out_ibc = glsneibc_sample(outlensed.to_dict('list'), cosmo)
...
out_ibc['t0'] = np.random.uniform(starting_date, ending_date, size=len(out_ibc['z']))

ibc = pd.DataFrame(out_ibc)
sample = GLSNe_sn1bc.from_data(ibc[ibc.Lensed==1])
...
outputfilename = path+'dset_ibc_'+str(size)+'_'+str(i)+'.pkl'
