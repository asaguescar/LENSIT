...
out_iip = glsneiip_sample(outlensed.to_dict('list'), cosmo)
...
out_iip['t0'] = np.random.uniform(starting_date, ending_date, size=len(out_iip['z']))

iip = pd.DataFrame(out_iip)
sample = GLSNe_sn2p_2005lc.from_data(iip[iip.Lensed==1])
...
outputfilename = path+'dset_iip_'+str(size)+'_'+str(i)+'.pkl'
